...
public struct SDL2ControllerGUID: Equatable, Hashable, CustomStringConvertible {
    ...
    public init?(_ guid: String, vendor: Int = 0, product: Int = 0) {
        ...
        guard
            CharacterSet(charactersIn: guid).subtracting(
    ...
    public init(vendorID: Int, productID: Int, hidVersion: Int, transport: Int, name: String) {
        ...
        func sdlCRC16(_ crc: UInt16, _ data: UnsafePointer<Int8>, _ len: Int) -> UInt16 {
            func crc16_for_byte(_ r: UInt8) -> UInt16 {
                ...
                for _ in 0 ..< 8 {
                    crc =
                        ((((UInt8(truncatingIfNeeded: crc) ^ r) & 1) != 0) ? 0xA001 : 0) ^ crc >> 1
                    r >>= 1
                }
                return crc
            }
            ...
            for i in 0 ..< len {
                crc =
                    crc16_for_byte(UInt8(truncatingIfNeeded: crc) ^ UInt8(bitPattern: data[i]))
                    & crc >> 8
            }
            return crc
        }

        self.guid = [transport, crcname, vendor, 0, product, 0, version, 0].withUnsafeBufferPointer(
            { pointer in
                var string: String = ""
...
struct SDL2ControllerMap {
    ...
    struct Value {
        ...
        init?(_ string: String) {
            ...
            guard let k = string.unicodeScalars.first(where: { kinds.contains($0) }) else {
                return nil
            }
    ...
    func symbols() -> GamePadSymbolMap {
        ...
        } else if comps.intersection([
            "Sony", "PlayStation", "PS", "PS5", "PS4", "PS3", "PS2", "PS1", "Vita", "PSP",
        ]).isEmpty == false {
        ...
        if comps.contains("8BitDo")
            && comps.intersection(["SF30", "SFC30", "NES30", "SN30"]).isEmpty == false
        {
            return .nintendoClassic
        }
...
class SDL2Database {
    ...
    init() throws {
        ...
        for line in string.components(separatedBy: "\n") {
            guard line.indices.contains(line.startIndex) && line[line.startIndex] != "#" else {
                continue
            }
