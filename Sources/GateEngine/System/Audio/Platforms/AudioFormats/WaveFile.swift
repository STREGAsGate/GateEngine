...
class WaveFile {
    ...
    func format() -> AudioBuffer.Format {
        if isIEEEFloat {
            ...
            return AudioBuffer.Format(channels: channelCount == 2 ? .stereo(interleved: true) : .mono, bitRate: .float32, sampleRate: Double(samplesPerSecond))
        }
        if bitsPerSample == 8 {
            return AudioBuffer.Format(channels: channelCount == 2 ? .stereo(interleved: true) : .mono, bitRate: .uint8, sampleRate: Double(samplesPerSecond))
            ...
            return AudioBuffer.Format(channels: channelCount == 2 ? .stereo(interleved: true) : .mono, bitRate: .int32, sampleRate: Double(samplesPerSecond))
        }else{
            fatalError("\(bitsPerSample)-bit \(channelCount)CH PCM Not Supported. Must be 8-bit, 16-bit, or 32-bit and 1 or 2 channels.")
        }
    }
    ...
    required init?(_ data: Data, context: AudioContext) {
        ...
        if data[20 ..< 22].boundTo(Int16.self) == 0x0001 || data[20 ..< 22].boundTo(Int16.self) == 0x0003 {
            ...
            var dataPos = 36
            while String(bytes: data[dataPos ..< dataPos + 4], encoding: .ascii) != "data" || dataPos > data.indices.upperBound {
                dataPos += 4
            }
            dataPos += 4
