...
extension AudioBuffer.Format {
    init(_ avFormat: AVAudioFormat) {
        self.channels = avFormat.channelCount == 2 ? .stereo(interleved: avFormat.isInterleaved) : .mono
...
internal class CABufferReference: AudioBufferBackend {
    ...
    required init(path: String, context: AudioContext, audioBuffer: AudioBuffer) {
        ...
        Task(priority: .utility) {
            do {
                guard let path = await Game.shared.platform.locateResource(from: path) else {throw GateEngineError.failedToLocate}
    ...
    func load(data: Data, format: AudioBuffer.Format) {
        ...
        let convertedData: Data = BufferConverter(data: data, format: format).reformat(as: newFormat)
        let channelLayout = AVAudioChannelLayout(layoutTag: newFormat.channels == .mono ? kAudioChannelLayoutTag_Mono : kAudioChannelLayoutTag_Stereo)!
        let avFormat = AVAudioFormat(commonFormat: .pcmFormatFloat32, sampleRate: newFormat.sampleRate, interleaved: false, channelLayout: channelLayout)
        
        let streamDesc = avFormat.streamDescription.pointee
        let frameCapacity = UInt32(convertedData.count) / streamDesc.mBytesPerFrame / (avFormat.isInterleaved ? 1 : avFormat.channelCount)
        ...
        @_transparent
        func writeData<T:Numeric>(_ data: Data, to pointer: UnsafePointer<UnsafeMutablePointer<T>>?) {
            if avFormat.isInterleaved == false {
                ...
                let channels = UnsafeMutableBufferPointer(start: pointer, count: Int(avFormat.channelCount))
                for index in channels.indices {
                    let channel = UnsafeMutableBufferPointer(start: channels[index], count: countPerChannel)
                    let start = countPerChannel * index
                    _ = data[start ..< start + countPerChannel].copyBytes(to: channel, count: countPerChannel)
                }
            }else{
                let pointer = UnsafeMutablePointer(mutating: pointer)
