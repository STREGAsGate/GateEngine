...
public struct RawGeometry: Codable, Equatable, Hashable {
    ...
    public func generateVerticies() -> [Vertex] {
        ...
        for vertexIndex in indicies.indices {
            ...
            vertices[vertexIndex].position = Position3(positions[start3], positions[start3 + 1], positions[start3 + 2])
            ...
            vertices[vertexIndex].normal = Direction3(normals[start3], normals[start3 + 1], normals[start3 + 2])
            vertices[vertexIndex].tangent = Direction3(tangents[start3], tangents[start3 + 1], tangents[start3 + 2])
            vertices[vertexIndex].color = Color(colors[start4], colors[start4 + 1], colors[start4 + 2], colors[start4 + 3])
        }
        return vertices
    }
    ...
    public func flipped() -> RawGeometry {
        return RawGeometry(triangles: self.generateTriangles().map({$0.flipped()}), optimizeDistance: nil)
    }
