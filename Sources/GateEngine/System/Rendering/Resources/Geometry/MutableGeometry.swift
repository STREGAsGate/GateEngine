...
public class MutableGeometry: Geometry {
    ...
    private func load() {
        ...
        Task {
            guard let cache = Game.shared.resourceManager.geometryCache(for: cacheKey) else {return}
            cache.geometryBackend = await Game.shared.resourceManager.geometryBackend(from: rawGeometry)
            Task {@MainActor in
                cache.state = .ready
            }
        }
