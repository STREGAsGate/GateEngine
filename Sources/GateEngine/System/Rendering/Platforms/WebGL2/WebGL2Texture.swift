...
class WebGL2Texture: TextureBackend {
    ...
    func replaceData(with data: Data, size: Size2, mipMapping: MipMapping) {
        ...
        if data.count == 36, let tagID = String(data: data, encoding: .utf8), let ele = document.getElementById(elementId: tagID), let image = HTMLImageElement(from: ele) {
            _ = document.body?.removeChild(child: image)
            let imageSource = TexImageSource.htmlImageElement(image)
            gl.texImage2D(target: GL.TEXTURE_2D, level: 0, internalformat: GLint(GL.RGBA), format: GL.RGBA, type: GL.UNSIGNED_BYTE, source: imageSource)
        }else{
            let data = JSTypedArray<UInt8>(data)
            gl.texImage2D(target: GL.TEXTURE_2D, level: 0, internalformat: GLint(GL.RGBA), width: GLsizei(size.width), height: GLsizei(size.height), border: 0, format: GL.RGBA, type: GL.UNSIGNED_BYTE, pixels: .uint8Array(data))
        }
        if case let .auto(levels) = mipMapping, levels > 1  {
            gl.texParameteri(target: GL.TEXTURE_2D, pname: GL.TEXTURE_MAX_LEVEL, param: GLint(levels))
