...
class WebGL2RenderTarget: RenderTargetBackend {
    ...
    func reshape() {
        if isWindow {
            ...
            context.texImage2D(target: GL.TEXTURE_2D, level: 0, internalformat: GLint(GL.RGBA), width: GLsizei(size.width), height: GLsizei(size.height), border: 0, format: GL.RGBA, type: GL.UNSIGNED_BYTE, pixels: nil)
    ...
    init(isWindow: Bool) {
        ...
        if isWindow {
            ...
            context.texImage2D(target: GL.TEXTURE_2D, level: 0, internalformat: GLint(GL.RGBA), width: GLsizei(size.width), height: GLsizei(size.height), border: 0, format: GL.RGBA, type: GL.UNSIGNED_BYTE, pixels: nil)
            context.texParameteri(target: GL.TEXTURE_2D, pname: GL.TEXTURE_MIN_FILTER, param: GLint(GL.LINEAR))
            context.texParameteri(target: GL.TEXTURE_2D, pname: GL.TEXTURE_MAG_FILTER, param: GLint(GL.NEAREST))
            context.texParameteri(target: GL.TEXTURE_2D, pname: GL.TEXTURE_WRAP_S, param: GLint(GL.CLAMP_TO_EDGE))
            context.texParameteri(target: GL.TEXTURE_2D, pname: GL.TEXTURE_WRAP_T, param: GLint(GL.CLAMP_TO_EDGE))
