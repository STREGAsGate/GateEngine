...
private func bakeFontBitmap(
    ...
    firstChar: Int32,
    charCount: Int32,
    charData: inout [CharData]
) -> Int32 {
    return fontData.withUnsafeBytes { fontData in
        return stbtt_BakeFontBitmap(
            fontData.baseAddress?.assumingMemoryBound(to: UInt8.self),
            offset,
            pixelHeight,
            &pixels,
            pixelsWidth,
...
@MainActor struct TTFFont: FontBackend {
    ...
    mutating func characterData(forKey key: Font.Key, character: Character) -> CharacterData {
        ...
        return CharacterData(
            texturePosition: FontQuad(
                min: Position2(Float(charData.x0), Float(charData.y0)),
    ...
    mutating func alignedCharacter(
        forKey key: Font.Key,
        character: Character,
        ...
        var pX = origin.x
        var pY = origin.y
        let result = getBakedQuad(
            characterData: characterData,
            pixelsWidth: Int32(textureSize.width),
            pixelsHeight: Int32(textureSize.height),
            charIndex: Int32(index),
            xPosition: &pX,
            yPosition: &pY
        )
        ...
        return AlignedCharacter(
            position: FontQuad(
                min: Position2(result.x0, result.y0),
                max: Position2(result.x1, result.y1)
            ),
            texturePosition: FontQuad(
                min: Position2(result.s0, result.t0),
    ...
    private mutating func populate(forPointSize pointSize: UInt, style: Font.Style) {
        ...
        let result = bakeFontBitmap(
            fontData: data,
            pixelHeight: Float(pointSize),
            pixels: &pixels,
            pixelsWidth: width,
            pixelsHeight: height,
            firstChar: 0,
