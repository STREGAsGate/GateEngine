...
public struct Material {
    ...
    public struct Channel: Equatable {
         ...
         */
        @MainActor
        public mutating func setSubRect(_ subRect: Rect2i) {
            ...
            let xRoundingOffset: Float = (1 / Float(texture.size.width)) * 0.5
            let yRoundingOffset: Float = (1 / Float(texture.size.height)) * 0.5
            self.scale = Size2(
                Float(subRect.size.width) / Float(texture.size.width),
                Float(subRect.size.height) / Float(texture.size.height)
            )
