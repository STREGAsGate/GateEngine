...
public class ShaderDocument: Identifiable {
    ...
    public func branch<T: ShaderValue>(if lhs: some Scalar, success: T, failure: T) -> T {
        return T(Operation(compare: lhs, success: success, failure: failure))
    }
    ...
    public static let cacheVersion: UInt32 = 1
    internal func documentIdentifierInputData() -> [Int] {
        fatalError("Must override")
    }
    lazy public private(set) var id: UInt32 = generateID()
    internal func generateID() -> UInt32 {
        let data = self.documentIdentifierInputData()
        return data.withUnsafeBytes({ bufferPointer in
            ...
            var crc: UInt32 = Self.cacheVersion ^ 0xffffffff
            for b: UInt8 in bufferPointer {
