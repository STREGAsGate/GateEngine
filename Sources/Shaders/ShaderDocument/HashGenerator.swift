...
//  
//
//  Created by Dustin Collins on 10/6/23.
//
...
struct HashGenerator {
    enum Seed: UInt64 {
        ...
        case valueScalar        = 8
        case valueSampler       = 9
        case valueOperation     = 10
        case valueMat4Array     = 11
        case valueMat4          = 12
        case valueMat3          = 13
    }
    
    static var generatedIDs: Set<UInt64> = []
    static var datas: Set<[Int]> = []
    internal static func generateID(_ data: [Int], seed: Seed) -> UInt64 {
        var data = data
        ...
        data.append(sum)
        data.append(Int(truncatingIfNeeded: seed.rawValue))
        ...
        let myCRC64 = data.withUnsafeBytes { bytes in
            var myCRC64: UInt64 = 0
            for b in bytes {
                myCRC64 = crcTable[Int((UInt8(truncatingIfNeeded: myCRC64) ^ b) & 0xFF)] ^ (myCRC64 >> 8)
            }
            return myCRC64 - seed.rawValue
        }

//        assert(generatedIDs.contains(myCRC64) == false)
//        generatedIDs.insert(myCRC64)
        assert(myCRC64 != 0)
        return myCRC64
    }
}
